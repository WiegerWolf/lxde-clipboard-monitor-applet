...
static gchar *shorten_history_label(const gchar *text) {
    ...
    if (char_count > HISTORY_PREVIEW_MAX_CHARS) {
        ...
        truncated = g_strndup(single_line, truncate_at - single_line);
        preview = g_strconcat(truncated, "...", NULL);
...
static void clipboard_monitor_free(gpointer user_data) {
    ClipboardMonitor *monitor = (ClipboardMonitor *)user_data;
...
static GtkWidget *clipboard_constructor(LXPanel *panel, config_setting_t *settings) {
    ...
    monitor->max_items = HISTORY_MAX_ITEMS;

    lxpanel_plugin_set_data(plugin, monitor, clipboard_monitor_free);
    ...
    gtk_label_set_width_chars(GTK_LABEL(monitor->label), 20);
    gtk_label_set_max_width_chars(GTK_LABEL(monitor->label), 20);
    ...
    gtk_widget_add_events(plugin, GDK_BUTTON_PRESS_MASK);
